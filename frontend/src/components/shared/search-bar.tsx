...
export function SearchBar({ className, onTrigger, ...props }: SearchBarProps) {
  return (
    <button
      onClick={onTrigger}
      className={cn(
        "flex h-10 w-full max-w-sm items-center justify-between rounded-xl border border-border bg-secondary/60 px-3 py-2 text-xs text-muted-foreground transition-all hover:bg-secondary hover:border-border cursor-pointer outline-none focus-visible:ring-2 focus-visible:ring-primary/50",
        className
      )}
      {...props}
    >
